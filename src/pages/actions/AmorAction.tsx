...
export default function useArmorAction(action: (req?: Request) => void): Action {
	...
	return {
		...
		node: <>
			{ selected.length === 2 &&  <Button color="primary" className="fixed bottom-[60px] block left-5" onPress={ () => action({ data: { first: selected[0], second: selected[1] } }) }>Bestätigen</Button> }
		</>
	}
}
